...
namespace Tangerine.UI.Timeline
{
	internal class TimelineRuler : Widget
	{
		...
		/// <summary>
		/// Timestamps color and text color.
		/// </summary>
				...
				public override void Render()
				{
					...
					for (; startPosition.X < Ruler.Width; startPosition.X += Ruler.SmallStep, stepIndex++) {
						if (stepIndex % Ruler.SmallStepsPerBig == 0) {
							...
							var time = string.Format("{0:0.0#}", stepIndex * smallStep / 1000);
							Renderer.DrawTextLine(startPosition + offsetText, time, 16, Ruler.TimestampsColor, 0);
						}
						else {

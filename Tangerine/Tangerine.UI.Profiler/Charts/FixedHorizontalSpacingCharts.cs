	...
	internal abstract class FixedHorizontalSpacingCharts : Widget, IChartsGroup
	{
		...
		protected FixedHorizontalSpacingCharts(Parameters parameters)
		{
			...
			AddNode(new Widget {
				...
				Clicked = () => {
					int visibleSlicesRange = (int)Mathf.Min(
						ControlPointsCount, Width / ControlPointsSpacing + 1);
					int visibleSlicesOffset = ControlPointsCount - visibleSlicesRange;
					int mousePosition = (int)LocalMousePosition().X;
					int visibleSliceIndex = (mousePosition + ControlPointsSpacing / 2) / ControlPointsSpacing;
					int sliceIndex = Math.Min(ControlPointsCount - 1, visibleSlicesOffset + visibleSliceIndex);
					SliceSelected?.Invoke(GetSlice(sliceIndex));
				}
			});
		}

...
namespace Lime.Profiler.Network
{
	internal class Client : IConnection
	{
		...
		/// <inheritdoc/>
		public void LazySend(Message message) => messageProcessor?.LazySend(message);

		/// <inheritdoc/>
		public void SendSynchronouslyFirstInQueue() => messageProcessor?.SendSynchronouslyFirstInQueue();

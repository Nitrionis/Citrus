...
namespace Lime.Graphics.Platform
{
	...
	public class ProfilingInfo
	{
		...
		public static ProfilingInfo Acquire(object material = null, int passIndex = 0) =>
			Acquire(RenderObject.CurrentNode, ReferenceEquals(RenderObject.CurrentManager, SceneNodeManager), material, passIndex);
#endif

		public static ProfilingInfo Acquire(object node, object manager, object material, int passIndex) =>
			Acquire(node, ReferenceEquals(manager, SceneNodeManager), material, passIndex);
